 ...
 */
abstract class R3Model_BC extends Model_BC {
    private static
    $rdos = [],
    $lifetimes = [];
    ...
    final public static function setLifeTime($time = 2, $unit = Model_BC::H){
    ...
    public static function query ($require = "0" , array $orderby = [['1', false, DataObject::SORT_REGULAR]], $range = 0, $format = Model_BC::LIST_AS_OBJ){
        $rdo = self::getRDO();
        $objs = [];
        $result = self::querySelect($rdo, $require, $orderby, $range);
        if($result){
            if($format===Model_BC::LIST_AS_ARR){
                return array_map(array('AF\Models\Model_BC', 'restoreroot'), $result->toArray());
            }
    ...
    protected function build($data, $posted = false){
        ...
        if($posted){
            $this->data = array_map(array('AF\Models\Model_BC', 'restoreroot'), $this->data);
    ...
    public function save (){
        ...
        if($this->posted){
            ...
            $data = array_map(array('AF\Models\Model_BC', 'replaceroot'), $data);
            ...
            $data = array_map(array('AF\Models\Model_BC', 'replaceroot'), $this->data);
