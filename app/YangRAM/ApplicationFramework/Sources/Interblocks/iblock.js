 ...
 * http://nidn.yangram.com/interblocks/
 *
 * Date 2017-04-06
 */
;
(function(global, undefined) {
    ...
    var name = 'Interblocks JavaScript Framework',
        version = '0.9.00',
        website = 'nidn.yangram.com/interblocks/',
        ...
        runtime = (function() {
            ...
            for (i = scripts.length - 1; i >= 0; i--) {
                if (scripts[i].hasAttribute('interblocks')) {
                    return {
                        Element: scripts[i],
    ...
    var slice = function(arrayLike, startIndex) {
        ...
        error = function(str) {
            throw "Interblocks Error: " + str;
        },

        /* 调式报错：只有Interblocks处于调试模式时，才会抛出相应的错误描述，否则返回一个布尔值 */
        debug = function(str) {
            if (useDebugMode) {
                error(str);
            } else {
        ...
        pandora = storage.pandora = (function() {
            ...
            return extend(pandora, {
                ...
                asyncBlocks: function(includes, callback) {
                    return iBlock(includes, callback, true);
                },
    ...
    var iBlock = function(includes, callback, blockname) {
        ...
        Block = declareClass({
            ...
            callback: function() {
                console.log('Interblocks has loaded some require libraries.');
            },
            loading: function() {
                var that = this,
                    filetype,
    ...
    if (!global.parent || global.parent == global) {
        global.console.log('[' + startTime.toLocaleString() + ']Interblocks Framework Start Working!');
    }
}(window));