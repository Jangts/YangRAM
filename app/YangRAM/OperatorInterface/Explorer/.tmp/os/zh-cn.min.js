RegApplication('EXPLORER', (__thisapp__, System, YangRAM, Using, Global, undefined) => {
    ...
    const PICS = function() {
        ...
        self.getItemInfo('img', id, function(info) {
            ...
            html += '</v>';
            var popup = YangRAM.$('.browse-popup', __thisapp__.view)[0] || YangRAM.create('v', __thisapp__.view, { className: 'browse-popup', });
    ...
    const WAVS = function() {
        ...
        self.getItemInfo('wav', id, function(info) {
            ...
            self.palyer = YangRAM.$('.browse-popup', __thisapp__.view)[0] || YangRAM.create('v', __thisapp__.view, { className: 'browse-popup', });
    ...
    const CTX_MENU_HANDLERS = {
        OpenFolder() {},
        MoveTo() { __thisapp__.API.BROWSER_TRIGGERS.MoveSelected.call(this); },
        CopyURL() {
        ...
        CopyCode() { var type = YangRAM.attr(this, 'x-type'); var id = YangRAM.attr(this, 'x-id'); var suffix = YangRAM.attr(this, 'x-suffix'); var src = '//' + window.location.host + YangRAM.RequestDIR + 'files/' + type + '/' + id + '.' + suffix; if (type == 'img') { YangRAM.API.TXT.copy('<img src="' + src + '"/>'); }; if (type == 'wav') { YangRAM.API.TXT.copy('<audio src="' + src + '">' + __('WORDS')("Not Support") + '</audio>'); }; },
        Replace() {
            var that = this;
            ...
            YangRAM.tools.pickFiles({
                ...
                maxsize: 1024 * 1024 * 20,
                    ...
                    if (preg.test(files[0].name)) {
                        var p = YangRAM.tools.showMagicCube(30000, function() { alert(YangRAM.API.TXT.local('COMMON')('WORDS')("Timeout")); });
                    ...
                    } else { alert(__('WORDS')("Different Formats!")); }
                },
                fail(file, errtype) {
                    switch (errtype) {
                        case 0:
        ...
        Delete() { __thisapp__.Handlers.API.BROWSER_TRIGGERS.DeleteSelected(this); }
    };
    const UPLOADER = function(files) {
        if (privates.uploading) { alert(__('WORDS')("Uploader Already In Working, Please Wait For The End Of This Operation!")); } else {
            if (privates.uploadingState) { privates.toBeupload = YangRAM.API.util.arr.merge(privates.toBeupload, files); } else {
    ...
    const GET_FILE_SIZE = function(size) {
        if (size > 1024 * 1024 * 1204 * 1204) { return (size / (1024 * 1024 * 1204 * 1204)).toFixed(2) + 'T'; }
        if (size > 1024 * 1024 * 1204) { return (size / (1024 * 1024 * 1204)).toFixed(2) + 'G'; }
    ...
    const UPLOADER_START = function() {
        ...
        for (var i = 0; i < privates.toBeupload.length; i++) { UPLOADER_TRANSFER(privates.toBeupload[i], __thisapp__.$('.uploader-content .data-status')[i]); }
    };
    const UPLOADER_TRANSFER = function(file, status) {
        __thisapp__.upload(file, {
            fldid: __thisapp__.currentFolder,
            before(data) { status.innerHTML = data.responseText; },
            progress(data) { if (data.lengthComputable) { status.innerHTML = Math.round(data.loaded * 100 / data.total) + '%'; }; },
            after(data) { status.innerHTML = data.responseText; },
            done(data) {
                try {
    ...
    const UPLOADER_LISTEN = function() {
        if (privates.uploading && privates.uploadedNum + privates.uploadedFailNum == privates.toBeuploadNum) {
            ...
            if (privates.uploadedFailNum > 0) { content += __('WORDS')("But There Are Some Files Uploaded Failed."); }
            YangRAM.API.MSG.notice({ appId: __thisapp__.appid, title: __('WORDS')("Upload Complete"), content: content });
        }
    };
    ...
    const self = {
        ...
        menuOnHigabar: { appname: __LANG__.APPNAME, menus: { 'Common': [{ title: __('MENUS')("Default Page"), state: 'on', handler: 'OpenDefault' }, { title: __('MENUS')("Refresh"), state: 'on', handler: 'ReLoad' }], 'View': [{ title: 'Sleep', state: 'on', handler: 'Sleep' }, { title: 'Center', state: 'off', handler: 'Center' }, { title: 'Cover', state: 'off', handler: 'Cover' }, { title: 'Close', state: 'on', handler: 'Close' }], 'Help': [{ title: YangRAM.API.TXT.local('I4PLAZA')('MENUS')("YangRAM Official Website"), state: 'on', handler: 'GotoYangRAM' }, { title: YangRAM.API.TXT.local('I4PLAZA')('MENUS')("Forum"), state: 'on', handler: 'GotoForum' }, { title: YangRAM.API.TXT.local('I4PLAZA')('MENUS')("Developer Center"), state: 'on', handler: 'GotoDevCnt' }, { title: YangRAM.API.TXT.local('I4PLAZA')('MENUS')("About YangRAM"), state: 'on', handler: 'GotoAbout' }], } },
        menuOnFolder: [
            [{ title: __('MENUS')("Open Folder"), state: 'on', handler: CTX_MENU_HANDLERS.OpenFolder }, { title: __('MENUS')("Move To"), state: 'on', handler: CTX_MENU_HANDLERS.MoveTo }, { title: __('MENUS')("Rename"), state: 'on', handler: CTX_MENU_HANDLERS.Rename }, { title: __('MENUS')("Delete Folder"), state: 'on', handler: CTX_MENU_HANDLERS.Delete }]
        ],
        menuOnSPC: [
            [{ title: __('MENUS')("Delete Content"), state: 'on', handler: CTX_MENU_HANDLERS.Delete }]
        ],
        menuOnDocument: [
            [{ title: __('MENUS')("Move To"), state: 'on', handler: CTX_MENU_HANDLERS.MoveTo }, { title: __('MENUS')("Copy URL"), state: 'on', handler: CTX_MENU_HANDLERS.CopyURL }, { title: __('MENUS')("Replace"), state: 'on', handler: CTX_MENU_HANDLERS.Replace }, { title: __('MENUS')("Rename"), state: 'on', handler: CTX_MENU_HANDLERS.Rename }, { title: __('MENUS')("Delete Documents"), state: 'on', handler: CTX_MENU_HANDLERS.Delete }]
        ],
        menuOnPicture: [
            [{ title: __('MENUS')("Preview"), state: 'on', handler: PICS }, { title: __('MENUS')("Move To"), state: 'on', handler: CTX_MENU_HANDLERS.MoveTo }, { title: __('MENUS')("Copy URL"), state: 'on', handler: CTX_MENU_HANDLERS.CopyURL }, { title: __('MENUS')("Copy Code"), state: 'on', handler: CTX_MENU_HANDLERS.CopyCode }, { title: __('MENUS')("Replace"), state: 'on', handler: CTX_MENU_HANDLERS.Replace }, { title: __('MENUS')("Rename"), state: 'on', handler: CTX_MENU_HANDLERS.Rename }, { title: __('MENUS')("Delete Documents"), state: 'on', handler: CTX_MENU_HANDLERS.Delete }]
        ],
        menuOnText: [
            [{ title: __('MENUS')("Preview"), state: 'on', handler: TXTS }, { title: __('MENUS')("Move To"), state: 'on', handler: CTX_MENU_HANDLERS.MoveTo }, { title: __('MENUS')("Copy URL"), state: 'on', handler: CTX_MENU_HANDLERS.CopyURL }, { title: __('MENUS')("Replace"), state: 'on', handler: CTX_MENU_HANDLERS.Replace }, { title: __('MENUS')("Rename"), state: 'on', handler: CTX_MENU_HANDLERS.Rename }, { title: __('MENUS')("Delete Documents"), state: 'on', handler: CTX_MENU_HANDLERS.Delete }]
        ],
        menuOnAudio: [
            [{ title: __('MENUS')("Play"), state: 'on', handler: WAVS }, { title: __('MENUS')("Move To"), state: 'on', handler: CTX_MENU_HANDLERS.MoveTo }, { title: __('MENUS')("Copy URL"), state: 'on', handler: CTX_MENU_HANDLERS.CopyURL }, { title: __('MENUS')("Copy HTML Code"), state: 'on', handler: CTX_MENU_HANDLERS.CopyCode }, { title: __('MENUS')("Replace"), state: 'on', handler: CTX_MENU_HANDLERS.Replace }, { title: __('MENUS')("Rename"), state: 'on', handler() {} }, { title: __('MENUS')("Delete Documents"), state: 'on', handler: CTX_MENU_HANDLERS.Delete }]
        ],
        renameItem() {
            if (YangRAM.attr(this.parentNode, 'x-type') != 'spc') {
                if (YangRAM.attr(this.parentNode, 'readonly') != '' && YangRAM.attr(this, 'contenteditable') != '') {
                    ...
                    this.onblur = function() {
                        ...
                        if (newName != orgName) {
                            if (newName.match(/(<|>|\/|\\|\||:|\"|\*|\?)/)) { alert(__('WORDS')('Can Not Contain <>/\|:"*?'), function() { that.innerHTML = orgName; }); } else if (newName.length > 50) { alert(__('WORDS')("Can Not More Than 50 Words!"), function() { that.innerHTML = orgName; }); } else if (newName.length < 1) { alert(__('WORDS')("Can Not Empty!"), function() { that.innerHTML = orgName; }); } else {
                                ...
                                var itemNames = { 'img': __('NAMES')('img'), 'txt': __('NAMES')('txt'), 'vod': __('NAMES')('vod'), 'wav': __('NAMES')('wav'), 'folder': __('NAMES')('folder'), 'doc': __('NAMES')('doc'), 'spc': YangRAM.attr(this.parentNode, 'itemname') }
                                ...
                                YangRAM.set({
                                    ...
                                    data: data,
                                        ...
                                        if (txt == '<ERROR>' || txt.match('PHP Notice:')) { alert(YangRAM.API.TXT.local('COMMON')('WORDS')("Something Wrong!"), function() { that.innerHTML = orgName; }); } else if (txt == '<ERROR404>') {
                                            alert(__('WORDS')("File Not Exists!"));
                                            __thisapp__.refresh();
                                        } else {
                                            that.innerHTML = txt;
                                            YangRAM.attr(that.parentNode, 'name', txt).API.MSG.notice({ appId: __thisapp__.appid, title: __('WORDS')("Rename Success"), content: __('WORDS')('Rename_Success')(itemNames[type]) });
                                        }
                                        that.onblur = null;
                                    },
                                    fail(txt) {
                                        console.log(txt);
                                        YangRAM.tools.hideMagicCube(timeout);
                                        alert(YangRAM.API.TXT.local('COMMON')('WORDS')("Network Error!"), function() { that.innerHTML = orgName; });
                                        that.onblur = null;
                                    }
                                });
                                var timeout = YangRAM.tools.showMagicCube(30000, function() { alert(YangRAM.API.TXT.local('COMMON')('WORDS')("Operate Failed")); });
                            }
                        }
            ...
            } else { YangRAM.API.MSG.notice({ appId: __thisapp__.appid, title: __('WORDS')("Rename Prohibited"), content: __('WORDS')("Cannot Rename for a Preset Content!") }); }
        },
        itemTags: { a(type, id, name, sfix) { var src = YangRAM.RequestDIR + 'files/' + type + '/' + id + '.' + sfix; return '<a href="' + src + '">' + name + '</a>'; }, folder() { return ''; }, pic(type, id, name, sfix) { var src = YangRAM.RequestDIR + 'files/img/' + id + '.' + sfix; return '<img src="' + src + '" alt="' + name + '" />'; }, wav(type, id, name, sfix) { var src = YangRAM.RequestDIR + 'files/wav/' + id + '.' + sfix; return '<audio src="' + src + '"><a href="' + src + '?download">' + name + '</a></audio>'; }, vod(type, id, name, sfix) { var src = YangRAM.RequestDIR + 'files/vod/' + id + '.' + sfix; return '<video src="' + src + '"><a href="' + src + '?download">' + name + '</a></video>'; } },
        handleSmartianOrder() {
            var itemtype = arguments[1];
            var itemid = arguments[2];
            if (itemtype && itemid) {
        ...
        getchosenFolder(event) { var elem = event.relatedTarget  ||  event.srcElement  ||  event.target  || event.currentTarget; if (elem.tagName == 'item') { return elem; } else { return YangRAM.API.dom.closest(elem, 'item'); } },
        showDialogOfFolderList(txt) {
            YangRAM.tools.showDialog({ title: __('WORDS')("Move folders and files to"), appid: __thisapp__.appid, css: 'dialog', height: 400, control: [{ name: YangRAM.API.TXT.local('COMMON')('WORDS')("Cancel"), href: 'CloseDialog', args: '' }, { name: YangRAM.API.TXT.local('COMMON')('WORDS')("Confirm"), href: 'MoveSelectedTochosenFolder', args: '' }] }, function() {
                            ...
                            if (childList.html() == '') {
                                ...
                                YangRAM.set({
                                    ...
                                    data: data,
                                        ...
                                        if (txt == '<ERROR>' || txt.match('PHP Notice:')) { alert(YangRAM.API.TXT.local('COMMON')('WORDS')("Something Wrong!")); } else if (txt == '<ERROR404>') { alert(__('WORDS')("Folder Not Exists")); } else {
                                            childList.html(txt);
                                            setTimeout(function() { dialogMain.scrollBAR.resize(); }, 0);
                                        }
                                    },
                                    fail(txt) {
                                        console.log(txt);
                                    ...
                                    always(txt) { YangRAM.tools.hideMagicCube(); }
                                });
        ...
        eventHandlers: {
            ...
            'main content .item[x-type=img]': { 'click' (event) { var elem = event.relatedTarget  ||  event.srcElement  ||  event.target  || event.currentTarget; if (elem.className != "sele" && elem.className != "rplc" && (elem.className != "name" || YangRAM.attr(elem.parentNode.parentNode.parentNode, 'x-type') == 'list') && elem.tagName != "INPUT") { PICS.call(this); } } },
            'main content .item[x-type=txt]': { 'click' (event) { var elem = event.relatedTarget  ||  event.srcElement  ||  event.target  || event.currentTarget; if (elem.className != "sele" && elem.className != "rplc" && (elem.className != "name" || YangRAM.attr(elem.parentNode.parentNode.parentNode, 'x-type') == 'list') && elem.tagName != "INPUT") { TXTS.call(this); } } },
            'main content .item[x-type=wav]': { 'click' (event) { var elem = event.relatedTarget  ||  event.srcElement  ||  event.target  || event.currentTarget; if (elem.className != "sele" && elem.className != "rplc" && (elem.className != "name" || YangRAM.attr(elem.parentNode.parentNode.parentNode, 'x-type') == 'list') && elem.tagName != "INPUT") { WAVS.call(this); } } },
            'main content .item': {
                'rclick' (event) {
            ...
            'main content .item .sele': { 'click' (event) { YangRAM.toggleClass(this.parentNode, 'selected'); } },
            'main content .item .rplc': { 'click' (event) { CTX_MENU_HANDLERS.Replace.call(this.parentNode); } },
            'main .main-content[x-type=tile] .name': { 'click' (event) { self.renameItem.call(this); } },
            ...
            '.uploader-startbtn': { 'mousedown' (event) { if (privates.toBeupload.length > 0) { UPLOADER_START(); } else { alert(__('WORDS')("Please Select Files To Be Upload.")); } } },
            '.uploader-cancelbtn': { 'mousedown' (event) { if (privates.uploading) { alert(__('WORDS')("Uploader Already In Working, Can Not Be Canceled!")); } else { UPLOADER_HIDE(); } } },
            '.uploader_completebtn': { 'mousedown' () { __thisapp__.refresh(); } },
            ...
            '.browse-popup .browse-txt-content': { 'mousewheel' () { document.onmousewheel == null; } }
        }
    };
    const pm_5919526128523 = { palyer: null, toBeupload: [], toBeuploadNum: 0, uploadedNum: 0, uploadedFailNum: 0, uploadingState: false, uploading: false, uploaderScrollBAR: null };
    const pm_5919526128527 = {
        ...
        foldersList: '',
        API: {
            HIGHBAR_HANDLERS: { OpenDefault() { this.loadURI('default'); }, refresh() { this.refresh(); }, Cover() { this.setFullScreenView(); }, Center() { this.setCenteredView(); }, Sleep() { YangRAM.API.APP.sleep(this.appid); }, Close() { YangRAM.API.APP.close(this.appid); } },
            ...
            BROWSER_TRIGGERS: {
                CreateFolder() {
                    ...
                    YangRAM.set({
                        ...
                        data: data,
                            ...
                            if (txt == '<SUCCESS>') {
                                __thisapp__.refresh();
                                YangRAM.API.MSG.notice({ title: __('WORDS')("Create Success"), content: __('WORDS')("A New Folder Has Been Create Successfully!") });
                            } else if (txt == '<NO_PERMISSIONS>') { alert(YangRAM.API.TXT.local('COMMON')('WORDS')("No Permissions!")); } else { alert(YangRAM.API.TXT.local('COMMON')('WORDS')("Something Wrong!")); }
                        },
                        fail(txt) {
                            console.log(txt);
                            alert(YangRAM.API.TXT.local('COMMON')('WORDS')("Network Error!"));
                ...
                SelectAll() { __thisapp__.$('main content .item').toggleClass('selected', true); },
                InvertSelection() { __thisapp__.$('main content .item').toggleClass('selected'); },
                DeselectAll() { __thisapp__.$('main content .item').toggleClass('selected', false); },
                DeleteSelected() {
                    ...
                    if (selects.length > 1) {
                        ...
                        var listen = function() {
                            if (success + failed + pass == selects.length) {
                                if (success > 0) {
                                    YangRAM.API.MSG.notice({ appId: __thisapp__.appid, title: __('WORDS')("Delete Success"), content: __('WORDS')('Delete_Success_Multiple')(success) });
                                    __thisapp__.refresh();
                                } else { alert(__('WORDS')("Delete Failed!")); }
                            }
                        }
                    ...
                    } else if (selects.length == 1) { if (YangRAM.attr(selects[0], 'readonly') != '') { __thisapp__.removeItem(selects[0]); } else { alert(__('WORDS')("Items Readonly")); } } else { alert(__('WORDS')("Please select items to be operate.")); }
                },
                MoveSelected() {
                    var selects = __thisapp__.$('main content .item.selected');
                    if (selects.length > 0) {
                        if (__thisapp__.foldersList == '') {
                            YangRAM.set({
                                url: __thisapp__.__dirs.getter + 'folders/roots/',
                                    ...
                                    if (txt == '<ERROR>' || txt.match('PHP Notice:')) {
                                        alert(YangRAM.API.TXT.local('COMMON')('WORDS')("Something Wrong!"));
                                        console.log(txt);
                                    } else { self.showDialogOfFolderList(txt); }
                                },
                                fail(txt) {
                                    alert(YangRAM.API.TXT.local('COMMON')('WORDS')("Network Error!"));
                                    console.log(txt);
                            ...
                            YangRAM.tools.showMagicCube();
                        } else { self.showDialogOfFolderList(__thisapp__.foldersList); }
                    } else { alert(__('WORDS')("Please select items to be operate.")); }
                },
                CopyHTMLCodes() {
                    var selects = __thisapp__.$('main content .item.selected');
                    if (selects.length > 0) {
                ...
                CloseDialog() { YangRAM.tools.hideDialog(); },
                MoveSelectedTochosenFolder() {
                    ...
                    __thisapp__.$('main content .item.selected').each(function() {
                        ...
                        YangRAM.set({
                            ...
                            data: data,
                            done(txt) { if (txt == '<SUCCESS>') { __thisapp__.refresh(); } else if (txt == '<SELF>') { alert(__('WORDS')("Can Not Move To Self")); } else if (txt == '<CHILD>') { alert(__('WORDS')("Can Not Move To Child Folder")); } else if (txt == '<!EXIST>') { alert(__('WORDS')("Target Folder Not Exist")); } else { alert(YangRAM.API.TXT.local('COMMON')('WORDS')("Something Wrong!")); } },
                            fail(txt) {
                                console.log(txt);
                                alert(YangRAM.API.TXT.local('COMMON')('WORDS')("Network Error!"));
                            },
                            always() { YangRAM.tools.hideMagicCube(); }
                        });
                        YangRAM.tools.showMagicCube();;
                    });
                }
        ...
        onlaunch(href, folder) {
            if (typeof href == 'string') {
                href = href.split('?')[0];
                if (folder) { this.currentFolder = folder; } else { var REQUEST = href.replace(/\/+/, '/').split('/'); if (REQUEST[0] == 'src' && REQUEST[2] != '') { this.currentFolder = parseInt(REQUEST[2]); } else { this.currentFolder = 6; } }
                href += '?lt=' + this.listType + '&lo=' + this.listOrder;
                this.open(href, () => { if (privates.toBeupload.length) { UPLOADS_LISTER(privates.toBeupload); } });
            }
            return this.resize();
        },
        ...
        removeItem(elem, doneCallback, failCallback) {
            ...
            var doneCallback = doneCallback || function(txt) {
                if (txt.match('PHP Notice:')) { alert(YangRAM.API.TXT.local('COMMON')('WORDS')("Something Wrong!")); } else if (txt == '<Deleted>') { alert(__('WORDS')("<Deleted>")(itemName)); } else if (txt == '<ReadOnly>') { alert(__('WORDS')("<ReadOnly>")(itemName)); } else if (txt == '<CAN_NOT_FIND>') { alert(__('WORDS')("<CAN_NOT_FIND>")(itemName)); } else if (txt == '<SUCCESS>') {
                    YangRAM.API.MSG.notice({ appId: __thisapp__.appid, title: __('WORDS')("Delete Success"), content: __('WORDS')("Delete_Success")(itemName) });
                    __thisapp__.refresh();
                } else { alert(YangRAM.API.TXT.local('COMMON')('WORDS')("Unkonw Error!")); }
            }
            var failCallback = failCallback || function(txt) { alert(YangRAM.API.TXT.local('COMMON')('WORDS')("Network Error!")); }
            YangRAM.set({
                url: url,
                data: data,
                done(txt) {
            ...
            var timeout = YangRAM.tools.showMagicCube(30000, function() { alert('Timeout'); });
        },
        onafterresize() {
        ...
        main() { this.regContextMenus('ctx-fld', self.menuOnFolder).regContextMenus('ctx-set', self.menuOnSPC).regContextMenus('ctx-doc', self.menuOnDocument).regContextMenus('ctx-vod', self.menuOnDocument).regContextMenus('ctx-txt', self.menuOnText).regContextMenus('ctx-img', self.menuOnPicture).regContextMenus('ctx-wav', self.menuOnAudio).listenEvents(self.eventHandlers).loadStyle(function() { setTimeout(function() { __thisapp__.regHeadBar(self.menuOnHigabar).open(__thisapp__.__temp.href, () => { __thisapp__.setFullScreenView(); }); }, 1500) }) }
    };
    const privates = { palyer: null, toBeupload: [], toBeuploadNum: 0, uploadedNum: 0, uploadedFailNum: 0, uploadingState: false, uploading: false, uploaderScrollBAR: null };
