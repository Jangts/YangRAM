    ...
    private static function conn($options){
        if(is_numeric($options)&&isset(self::$conns[$options])){
            if(self::$conns[$options]['instance']){
                ...
                include_once(PATH_TNI.'DBAL/Drivers/'.self::$conns[$options]['driver'].'.php');
			    $class = 'Tangram\DBAL\Drivers\\'.self::$conns[$options]['driver'];
                return self::$conns[$options]['instance'] = $class::instance(self::$conns[$options]['options']);
            }
        }elseif(is_array($options)&&$options['driver']&&is_file(PATH_TNI.'DBAL/Drivers/'.$options['driver'].'.php')){
