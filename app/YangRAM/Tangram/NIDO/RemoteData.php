...
final class RemoteData {
    ...
    public static function gets(array $urls, $format = RemoteData::JSN){
        $objs = [];
        foreach($urls as $url){
    ...
    $headers,
    $data = [
        'type'  =>  'EmptyRemoteData',
        'value' =>  NULL
    ],
    $timeout = 30;

    public function __construct($url, $params = NULL, $format = RemoteData::JSN, array $headers = []) {
        $this->request = self::buildUrl($url, $params);
        $this->format = $format;
        $this->readonly = true;
        $this->headers = $headers;
        $this->headers['User-Agent'] = RemoteData::UA_MOZ_WIN;
    }
    ...
    public function read(){
        if(extension_loaded('CURL')){
            ...
            if(function_exists('get_headers')){
                ...
                $response = [
                    'type'  =>  'ErrorRemoteData',
                    'emsg'  =>  'Your I4s Not Support RemoteData.'
                ];
            }
        }
