		...
		if(count($array)>0){
			...
			foreach($array as $row){
				...
				$data["rows"][] = array(
					...
					'modi w70'	=>	'<click href="trigger://'.AI_CURR.'::CopyPageId" args="'.$row["pid"].'">'.$localdict['attrs']["modi"].'</click>',
	...
	public function render($posi){
		...
		if($posi=='list'){
			if(isset($this->data['list'])){
				return OIML::sheet($this->data['list'], 'lightpurple');
			}
			return '<list type="sheet" class="lightpurple"><vision class="tips">This Category Is Empty</vision></list>';
		}
		if($posi=='page'){
			return OIML::paging($this->data['page'], 'lightpurple');
		}
		return '';
	}
}