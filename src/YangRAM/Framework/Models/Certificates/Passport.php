...
final class Passport {
    ...
    private function check(){
        ...
        if(defined('_TEST_MODE_')&&_TEST_MODE_&&isset($REQUEST->ARGS->t_signas)){
            ...
            $this->account = UserAccount($username, Passport::QUERY_UID);
            Passport::$uid = $this->account->uid;
        ...
        if($this->checked===false){
            ...
            $this->account = new UserAccount($this->certificate['USER']);
            Passport::$uid = $this->account->uid;
            $this->guest = new Guest();
            $this->checked = true;
        }
    ...
    public function checkLoginPassword($identification, $password, $method = 2){
        ...
        if($identification){
            ...
            if($account){
                $this->account = new UserAccount($account, Passport::QUERY_LESS);
                Passport::$uid = $this->account->uid;
