...
class Tools {
    ...
    public static function buildCache(){
        ...
        for ($a=1; $a <= Spec_VERSION_MAX; $a++){
            ...
            if (QR_IMAGE){
                $fileName = QR_CACHE_URI.'frame_'.$a.'.png';
    ...
    public static function log($outfile, $err){
        if (QR_LOG_URI !== false){
            if ($err != ''){
                if ($outfile !== false){
                file_put_contents(QR_LOG_URI.basename($outfile).'-errors.txt', date('Y-m-d H:i:s').': '.$err, FILE_APPEND);
                } else {
                file_put_contents(QR_LOG_URI.'errors.txt', date('Y-m-d H:i:s').': '.$err, FILE_APPEND);
                }
            }    
        }
    }
